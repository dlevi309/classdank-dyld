...
static NSString *  parseImage(char *image,BOOL writeToDisk,NSString *outputDir,BOOL getSymbols,BOOL isRecursive,BOOL buildOriginalDirs,BOOL simpleHeader,BOOL skipAlreadyFound){
	...
	*/
    ...
    if (![lastComponent respondsToSelector:@selector(rangeOfString:)]){
       NSLog(@"BROOOOOOOO");
    }
	...
	for (unsigned i=0; i<count; i++){
        ...
        if (![classNameNSToRelease respondsToSelector:@selector(rangeOfString:)]){
            NSLog(@"BROOOOOOOO classNameNSToRelease");
            continue;
        }
        ...
        if (![comparestr respondsToSelector:@selector(rangeOfString:)]){
            NSLog(@"BROOOOOOOO comparestr");
            continue;
        }
        ...
        if (![classNameNS respondsToSelector:@selector(rangeOfString:)]){
            NSLog(@"BROOOOOOOO classNameNS");
            continue;
        }
